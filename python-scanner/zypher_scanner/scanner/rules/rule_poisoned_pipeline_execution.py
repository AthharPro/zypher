...
class RulePoisonedPipelineExecution(BaseRule):
        ...
        for step_index, step in enumerate(steps):
            if "run" in step and isinstance(step["run"], str):
                ...
                for pattern in self.unsafe_input_patterns:
                    if re.search(pattern, run_command):
                        # Check if the unsafe input is used with dynamic command execution
    ...
    def _find_run_command_line(self, file_lines: List[str], run_command: str, step: Dict[str, Any]) -> int:
        ...
        # Split the run command into lines for multi-line commands
    ...
    def _is_in_command_context(self, command: str, variable: str) -> bool:
        ...
        # Look for patterns that suggest command execution
        command_contexts = [
            f"eval.*{re.escape(variable)}",
            f"exec.*{re.escape(variable)}",
            f"bash.*{re.escape(variable)}",
            f"sh.*{re.escape(variable)}",
            f"`.*{re.escape(variable)}.*`",
            f"\$\(.*{re.escape(variable)}.*\)"
        ]
        ...
        lines = command.split("\n")
        for line in lines:
            # If line starts with the variable or has the variable after a pipe, it might be a command
